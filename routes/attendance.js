...
//  POST /api/mark-in
// ─────────────────────────────────────────────
router.post("/mark-in", requireAuth, checkOfficeNetwork, async (req, res) => {
...
//  POST /api/mark-out
// ─────────────────────────────────────────────
router.post("/mark-out", requireAuth, checkOfficeNetwork, async (req, res) => {
  const userId = req.user.id;
  const today = todayStr();
  const now = new Date();
  ...
  try {
    ...
    if (!record) {
      return res
        .status(404)
        .json({ error: "No IN record for today — please mark IN first" });
    }
    if (record.outTime) {
